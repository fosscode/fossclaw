...
export function Composer({ sessionId }: { sessionId: string }) {
  ...
  const isArchived = sessionData?.archived ?? false;
  ...
  const canSend = text.trim().length > 0 && isConnected && !isArchived;
  ...
  return (
    <div className="shrink-0 border-t border-cc-border bg-cc-card px-2 sm:px-4 py-2 sm:py-3">
      <div className="max-w-3xl mx-auto">
        ...
        {isArchived && (
          <div className="mb-3 px-4 py-2.5 rounded-[10px] bg-cc-muted/10 border border-cc-muted/20 flex items-center gap-2.5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-cc-muted">
                This session is archived and read-only. The CLI process has exited.
              </p>
            </div>
          </div>
        )}
        ...
        <div className={`relative bg-cc-input-bg border rounded-[14px] overflow-visible transition-colors ${
          ...
          <textarea
            ...
            placeholder={
              isArchived
                ...
                : isConnected
                ? "Type a message... (/ for commands, ? for shortcuts)"
                : "Waiting for CLI connection..."
            }
            disabled={!isConnected || isArchived}
            rows={1}
          ...
          <div className="flex items-center justify-between px-2.5 pb-2.5">
            ...
            <button
              ...
              title={isArchived ? "Not available for archived sessions" : "Toggle mode (Shift+Tab)"}
            >
              {isPlan ? (
                <svg viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                title={isArchived ? "Not available for archived sessions" : "Upload image"}
              >
