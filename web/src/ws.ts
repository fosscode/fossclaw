...
/** Sessions intentionally disconnected — suppress auto-reconnect */
const intentionalDisconnects = new Set<string>();
/** Monotonic counter to detect stale WebSocket handlers after rapid switching */
let connectionGeneration = new Map<string, number>();
...
function handleMessage(sessionId: string, generation: number, event: MessageEvent) {
  // Drop messages from stale WebSocket connections (rapid session switching)
  const currentGen = connectionGeneration.get(sessionId) ?? -1;
...
export function connectSession(sessionId: string) {
  ...
  // Remove from intentional-disconnect set (e.g. reconnecting to same session)
  intentionalDisconnects.delete(sessionId);

  // Bump generation so any stale handlers from previous connections are ignored
  const gen = (connectionGeneration.get(sessionId) ?? 0) + 1;
  connectionGeneration.set(sessionId, gen);

  const store = useStore.getState();
  store.setConnectionStatus(sessionId, "connecting");

  // Clear stale messages/streaming from any prior connection to prevent
  // briefly showing the old session's history while new history loads
  ...
  ws.onmessage = (event) => handleMessage(sessionId, gen, event);
...
export function disconnectSession(sessionId: string) {
  ...
  // Invalidate any in-flight message handlers
  connectionGeneration.delete(sessionId);
